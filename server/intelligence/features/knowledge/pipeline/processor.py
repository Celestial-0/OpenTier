...
from features.knowledge.pipeline.cleaning import (
    CleaningStrategy,
    DocumentType,
    clean_with_strategy,
)
from features.knowledge.pipeline.chunker import TextChunk, chunk_text
from features.knowledge.repository import (
    DocumentRepository as DocumentStorage,
    JobRepository as JobStorage,
)
from features.knowledge.pipeline.validation import (
    ValidationError,
    sanitize_metadata,
...
class DocumentProcessor:
    ...
    def __init__(
        ...
        # Lazily initialized when RETRIEVAL_BACKEND=qdrant
        self._qdrant = None
        self._embedder = None
        self._sparse = None
    ...
    async def process_document(
        ...
        try:
            ...
            try:
                ...
                # Vectors live in Qdrant; Postgres keeps the text
                # as the source of truth for backfill and audit.

                if self._qdrant is None:
                    self._qdrant = QdrantVectorStore()
                    self._embedder = build_embedding_gateway()
                    self._sparse = HashedSparseEncoder()

                dense_vecs = await self._embedder.embed_documents(chunk_texts)
                points = []
                for i, chunk in enumerate(chunk_objects):
                    s_idx, s_val = self._sparse.encode(chunk.content)
                    points.append(
                        {
                            "id": str(chunk.id),
                            "dense": dense_vecs[i],
                            "sparse": {"indices": s_idx, "values": s_val},
                            "user_id": str(getattr(db_doc, "user_id", "") or ""),
                            "is_global": bool(getattr(db_doc, "is_global", False)),
                            "document_id": str(db_doc.id),
                            "chunk_index": int(getattr(chunk, "chunk_index", i)),
                            "content": chunk.content,
                            "metadata": getattr(chunk, "metadata_", None) or {},
                            "document_title": str(getattr(db_doc, "title", "") or ""),
                            "source_url": str(getattr(db_doc, "source_url", "") or ""),
                        }
                    )
                await self._qdrant.upsert_chunks(points)
                logger.info(
                    f"Indexed {len(points)} chunks in Qdrant for document {db_doc.id}"
                )
