...
from features.chat.repository import (
    ChatRepository as ConversationStorage,
    MemoryRepository as MemoryStorage,
)
from shared.database.session import get_session
from features.billing import publish_chat_completed
...
def classify_error(error: Exception) -> str:
    ...
    Match on exception type (isinstance), not on stringified message.
    """
    # Type-based matching first (most reliable).
    import asyncio
    ...
    if "deadline" in error_str or "timed out" in error_str:
        return StreamErrorCode.TIMEOUT
...
# String-code → proto enum (intelligence_pb2.StreamErrorCode).
_PROTO_CODE_MAP = {
    StreamErrorCode.INTERNAL: 1,  # STREAM_ERROR_CODE_INTERNAL_ERROR
    StreamErrorCode.TIMEOUT: 2,  # STREAM_ERROR_CODE_TIMEOUT
    StreamErrorCode.RATE_LIMITED: 3,  # STREAM_ERROR_CODE_RATE_LIMITED
    StreamErrorCode.CONTEXT_TOO_LONG: 4,  # STREAM_ERROR_CODE_CONTEXT_TOO_LONG
    StreamErrorCode.MODEL_UNAVAILABLE: 5,  # STREAM_ERROR_CODE_MODEL_UNAVAILABLE
    StreamErrorCode.INVALID_REQUEST: 6,  # STREAM_ERROR_CODE_INVALID_REQUEST
    "DEADLINE_EXCEEDED": 7,  # STREAM_ERROR_CODE_DEADLINE_EXCEEDED
}


def to_proto_code(code_str: str) -> int:
    """Map a string error code onto the proto StreamErrorCode enum value."""
    return _PROTO_CODE_MAP.get(code_str, 0)  # 0 = UNSPECIFIED
...
class ChatService:
    ...
    async def send_message(
        ...
        async with get_session() as session:
            ...
            hold_key = metadata_dict.get("hold_key")
            ...
            # 3. Fetch Branch-aware History for Context
            # (R4.4: parity with StreamChat — respects parent_id lineage so
            # edited/regenerated branches stay isolated).
            ...
            # Memory extraction handled asynchronously by consumer via chat.completed.
            ...
            prompt_toks = int(metrics.get("prompt_tokens", 0))
            comp_toks = int(
                metrics.get("completion_tokens")
                or metrics.get("tokens_generated")
                or 0
            )

            # Emit billing event (never breaks the chat flow)
            try:
                from core.config import get_config as _gc

                resolved_model = config.get("model") if config and config.get("model") else _gc().llm.model
                await publish_chat_completed(
                    ...
                    model_slug=resolved_model,
                    sources_count=len(query_response.sources or []),
            ...
            chat_metrics = intelligence_pb2.ChatMetrics(
                tokens_used=comp_toks,
                prompt_tokens=prompt_toks,
                completion_tokens=comp_toks,
                latency_ms=float(metrics.get("total_time_ms", 0)),
                sources_retrieved=metrics.get(
                    "sources_retrieved", len(query_response.sources or [])
                ),
            )
        ...
        async with get_session() as session:
            ...
            async for chunk in self.query_pipeline.stream_response(
                ...
                if chunk["type"] == "sources":
                    ...
                    # Emit typed structured error to client with partial metrics
                    ...
                    if isinstance(error_data, Exception):
                        ...
                        # Already a string; strip a legacy "CODE: " prefix if present
                        error_message = str(error_data)
                        error_code = StreamErrorCode.INTERNAL

                    structured_error = f"{error_code}: {error_message}"
                    ...
                    metrics=intelligence_pb2.ChatMetrics(
                        ...
                        prompt_tokens=partial_metrics.get("prompt_tokens", 0),
                    ...
                    # Then emit the typed error chunk (v1.1.0 contract)
                    yield intelligence_pb2.ChatStreamChunk(
                        conversation_id=str(conv.id),
                        message_id=message_id,
                        typed_error=intelligence_pb2.StreamError(
                            code=to_proto_code(error_code),
            ...
            prompt_toks = int(all_metrics.get("prompt_tokens", 0))
            comp_toks = int(
                all_metrics.get("completion_tokens")
                or all_metrics.get("tokens_generated")
                or 0
            )
            ...
            # Yield terminal chunk with metrics only (oneof: metrics wins;
            # no token="" crutch per v1.1.0 contract documentation)
            yield intelligence_pb2.ChatStreamChunk(
                conversation_id=str(conv.id),
                message_id=message_id,
                metrics=chat_metrics,
            ...
            # Emit billing event (never breaks the stream flow)
            try:
                from core.config import get_config as _gc
    ...
    async def get_conversation(
        ...
        async with get_session() as session:
            ...
            # Keyset pagination — cursor is an opaque base64 (created_at,id) string (None = first page).
            messages = await storage.get_messages(
                conv_uuid, limit=limit + 1, cursor=cursor or None
            )
            logger.debug(
            ...
            # Calculate next cursor from the last returned message
            next_cursor = None
