...
class IntelligenceEngine:
    ...
    async def get_conversation(
        ...
        return await self.chat_service.get_conversation(
            conversation_id, user_id, limit=limit or 100, cursor=cursor
        )
    ...
    async def retrieve_context_internal(
        self, query: str, user_id: str, top_k: int = 5, doc_ids: Optional[list] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def generate_with_context_internal(
        self, prompt: str, context: list, user_id: str
    ) -> Dict[str, Any]:
        """
        ...
        async with get_session() as session:
            ...
            if job_id:
                ...
                if doc and user_id and doc.user_id != user_id:
                    logger.warning(
                        f"User {user_id} attempted to access resource {resource_id} owned by {doc.user_id}"
                    )
                    return intelligence_pb2.ResourceStatusResponse(
                        job_id="",
                        resource_id=resource_id,
            ...
            if resource_id:
                try:
                    chunks_created = await doc_storage.get_document_chunk_count(
        ...
        async with get_session() as session:
            ...
            if user_id:
                ...
                if doc and doc.user_id != user_id:
                    logger.warning(
                        f"User {user_id} attempted to delete resource {resource_id} owned by {doc.user_id}"
                    )
                    return False
