...
class ResourceService(intelligence_pb2_grpc.ResourceServiceServicer):
    ...
    async def AddResource(self, request, context):
        ...
        try:
            ...
            logger.debug(
                f"[{correlation_id}] AddResource: user={request.user_id}, type={content_type}"
            )
    ...
    async def GetResourceStatus(self, request, context):
        ...
        try:
            ...
            logger.debug(
                f"[{correlation_id}] GetResourceStatus: job={request.job_id}, resource={request.resource_id}"
            )
            ...
            status_code = classify_grpc_error(e)
            logger.error(
                f"[{correlation_id}] GetResourceStatus failed: {e}", exc_info=True
            )
            context.set_code(status_code)
    ...
    async def ListResources(self, request, context):
        ...
        try:
            ...
            logger.debug(
                f"[{correlation_id}] ListResources: user={request.user_id}, limit={request.limit}"
            )
            return await self.engine.list_resources(
                user_id=request.user_id, limit=request.limit or 20
            )
        except Exception as e:
            status_code = classify_grpc_error(e)
    ...
    async def DeleteResource(self, request, context):
        ...
        try:
            ...
            logger.debug(
                f"[{correlation_id}] DeleteResource: user={request.user_id}, resource={request.resource_id}"
            )
            ...
            status_code = classify_grpc_error(e)
            logger.error(
                f"[{correlation_id}] DeleteResource failed: {e}", exc_info=True
            )
            context.set_code(status_code)
    ...
    async def CancelIngestion(self, request, context):
        ...
        try:
            ...
            logger.debug(
                f"[{correlation_id}] CancelIngestion: user={request.user_id}, job={request.job_id}"
            )
            success, message = await self.engine.cancel_ingestion(
                job_id=request.job_id, user_id=request.user_id
            )
            return intelligence_pb2.CancelIngestionResponse(
                success=success, job_id=request.job_id, message=message
            )
        except Exception as e:
            status_code = classify_grpc_error(e)
            logger.error(
                f"[{correlation_id}] CancelIngestion failed: {e}", exc_info=True
            )
            context.set_code(status_code)
            context.set_details(f"CancelIngestion error: {str(e)}")
            return intelligence_pb2.CancelIngestionResponse(
                success=False, job_id=request.job_id, message=str(e)
            )
    ...
    async def ChunkedUpload(self, request_iterator, context):
        ...
        try:
            async for chunk in request_iterator:
                ...
                if chunks_received == 0:
                    ...
                    if metadata.total_size > MAX_CHUNKED_FILE_SIZE:
                        context.set_code(grpc.StatusCode.INVALID_ARGUMENT)
                        context.set_details(
                            f"File too large: {metadata.total_size} bytes (max: {MAX_CHUNKED_FILE_SIZE})"
                        )
                        return intelligence_pb2.ChunkedUploadResponse(
                    ...
                    logger.info(
                        f"[{correlation_id}] ChunkedUpload started: user={metadata.user_id}, "
                ...
                if not chunk.HasField("data"):
                    logger.warning(
                        f"[{correlation_id}] Chunk {chunk.chunk_index} has no data"
                    )
                    continue
                ...
                if chunk.chunk_index != chunks_received:
                    context.set_code(grpc.StatusCode.INVALID_ARGUMENT)
                    context.set_details(
                        f"Out of order chunk: expected {chunks_received}, got {chunk.chunk_index}"
                    )
                    return intelligence_pb2.ChunkedUploadResponse(
                        resource_id=resource_id,
                        status=intelligence_pb2.RESOURCE_STATUS_FAILED,
                ...
                if len(chunk.data) > MAX_CHUNK_SIZE:
                    context.set_code(grpc.StatusCode.INVALID_ARGUMENT)
                    context.set_details(
                        f"Chunk too large: {len(chunk.data)} bytes (max: {MAX_CHUNK_SIZE})"
                    )
                    return intelligence_pb2.ChunkedUploadResponse(
                        resource_id=resource_id,
                        status=intelligence_pb2.RESOURCE_STATUS_FAILED,
                ...
                logger.debug(
                    f"[{correlation_id}] Received chunk {chunk.chunk_index}, "
            ...
            if total_bytes != metadata.total_size:
                context.set_code(grpc.StatusCode.INVALID_ARGUMENT)
                context.set_details(
                    f"Size mismatch: received {total_bytes}, expected {metadata.total_size}"
                )
                return intelligence_pb2.ChunkedUploadResponse(
                    resource_id=resource_id,
                    status=intelligence_pb2.RESOURCE_STATUS_FAILED,
            ...
            logger.info(
                f"[{correlation_id}] ChunkedUpload complete: {chunks_received} chunks, {total_bytes} bytes"
            )
        ...
        try:
            if not check_deadline(context):
                ...
                return intelligence_pb2.SyncMetadataResponse(
                    success=False, sync_timestamp=int(time.time())
                )

            logger.info(
                f"[{correlation_id}] SyncResourceMetadata: user={request.user_id}, "
            ...
            status_code = classify_grpc_error(e)
            logger.error(
                f"[{correlation_id}] SyncResourceMetadata failed: {e}", exc_info=True
            )
            context.set_code(status_code)
