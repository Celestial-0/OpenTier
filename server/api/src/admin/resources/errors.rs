    ...
    fn into_response(self) -> Response {
        let (status, message) = match self {
            ResourceError::UnsupportedResourceType(ref t) => {
                (StatusCode::BAD_REQUEST, format!("Unsupported resource type: {}", t))
            }
            ResourceError::InvalidContent => {
                (StatusCode::BAD_REQUEST, "Invalid resource content".to_string())
            }
            ResourceError::InvalidUrl(ref e) => {
                (StatusCode::BAD_REQUEST, format!("URL validation failed: {}", e))
            }
            ResourceError::ContentTooLarge => {
                (StatusCode::PAYLOAD_TOO_LARGE, "Content too large".to_string())
            }
            ResourceError::ResourceNotFound => {
                (StatusCode::NOT_FOUND, "Resource not found".to_string())
            }
            ResourceError::AddResourceFailed => {
                (StatusCode::INTERNAL_SERVER_ERROR, "Failed to add resource".to_string())
            }
            ResourceError::ListResourcesFailed => {
                (StatusCode::INTERNAL_SERVER_ERROR, "Failed to list resources".to_string())
            }
            ResourceError::GetStatusFailed => {
                (StatusCode::INTERNAL_SERVER_ERROR, "Failed to get resource status".to_string())
            }
            ResourceError::DeleteResourceFailed => {
                (StatusCode::INTERNAL_SERVER_ERROR, "Failed to delete resource".to_string())
            }
            ResourceError::InvalidFilters => {
                (StatusCode::BAD_REQUEST, "Invalid filter parameters".to_string())
            }
            ResourceError::Unauthorized => {
                (StatusCode::FORBIDDEN, "Insufficient permissions".to_string())
            }
            ResourceError::GrpcError(ref e) => {
                (StatusCode::INTERNAL_SERVER_ERROR, format!("Service error: {}", e))
            }
