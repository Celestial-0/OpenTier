# Admin & RBAC

## RBAC Enforcement

Authorization is enforced at two layers:

1. **`auth_middleware`** — validates the session token and injects `role: Role` into request extensions. All `/admin` routes require this middleware.
2. **`require_admin`** — reads `role` from extensions and returns `403 Forbidden` if not `Role::Admin`. Zero additional DB queries.


<Mermaid chart={`
flowchart TD
    Request["Incoming /admin/* request"]
    AuthMiddleware["auth_middleware\nSELECT user_id + role FROM sessions\nWHERE token=$1 AND not expired"]
    RoleCheck{"role == Admin?"}
    Handler["Admin Handler"]
    Forbidden["403 Forbidden"]
    Unauthorized["401 Unauthorized"]

    Request --> AuthMiddleware
    AuthMiddleware -- "Token valid" --> RoleCheck
    AuthMiddleware -- "Token invalid/expired" --> Unauthorized
    RoleCheck -- "Yes" --> Handler
    RoleCheck -- "No (Role::User)" --> Forbidden
`} />


## Admin Stats

`GET /admin/stats` aggregates platform-wide metrics in a single handler, performing multiple DB queries:
- `COUNT(*) FROM users WHERE deleted_at IS NULL`
- `COUNT(*) FROM users WHERE created_at > NOW() - INTERVAL '24 hours'` (active today = created today, a proxy metric)
- `COUNT(*) FROM conversations`
- `COUNT(*) FROM messages` (API layer messages table)
- Count of intelligence resources via gRPC `ResourceService.ListResources`

## User Management

| Operation | Query |
|-----------|-------|
| `list_users` | `SELECT * FROM users ORDER BY created_at DESC LIMIT $1 OFFSET $2` with optional `ILIKE` search on `email` or `username` |
| `get_user` | `SELECT * FROM users WHERE id = $1` |
| `update_user_role` | `UPDATE users SET role = $1::user_role WHERE id = $2` |
| `delete_user` | `DELETE FROM users WHERE id = $1` (hard delete, cascades sessions/accounts) |

> **Note**: Admin `delete_user` is a **hard delete** (`DELETE FROM users`), whereas user self-deletion via `DELETE /user/delete-account` is a **soft delete** (`SET deleted_at = NOW()`). These two paths have different cascade behaviors.

## Resource Management

Resources (documents for RAG ingestion) are managed by the Intelligence service. The Rust admin handlers act as a proxy layer:


<Mermaid chart={`
sequenceDiagram
    participant Admin as Admin Browser
    participant Rust as Rust API
    participant Python as Python Intelligence
    participant DB as PostgreSQL + pgvector

    Admin->>Rust: POST /admin/resources resource_type content title is_global config
    Rust->>Rust: Map resource_type string to pb::ResourceType enum
    Rust->>Python: gRPC AddResource user_id=admin resource_id=uuid content type is_global
    Note over Rust,Python: Timeout: 3000s (50 min - large document ingestion)
    Python->>DB: INSERT document, chunk, embed, update ingestion_job
    Python-->>Rust: resource_id + job_id + status queued
    Rust-->>Admin: 202 resource_id + job_id + status

    Admin->>Rust: GET /admin/resources/id
    Rust->>Python: gRPC GetResourceStatus resource_id
    Python-->>Rust: status=processing progress_percent=45.0
    Rust-->>Admin: ResourceStatusResponse
`} />


**Resource types** (mapped from string to `pb::ResourceType` enum):
- `"text"` → `TEXT`
- `"markdown"` → `MARKDOWN`
- `"pdf"` → `PDF`
- `"html"` → `HTML`
- `"website"` → `WEBSITE` (triggers Playwright scraper)
- `"code"` → `CODE`

**Global resources** (`is_global: true`): searchable by all users in hybrid search. Global resources are typically set by admins for shared knowledge base documents.

## Weaknesses

1. **Role change inconsistency**: Changing a user's role via `PATCH /admin/users/{id}/role` updates `users.role` but does NOT update existing sessions' `role` column. Active sessions retain the old role until expiry (7 days) or re-login.

2. **Hard vs soft delete asymmetry**: Hard-deleting a user via admin bypasses the recovery mechanism available through `POST /auth/recover-account`.

3. **No audit log**: Admin operations (role changes, user deletions, resource additions) are not logged to an audit trail. Only `TraceLayer` HTTP logs exist.

4. **gRPC timeout**: Resource ingestion timeout is 3000s (50 min). A failure mid-ingestion may leave an `ingestion_job` in `processing` state with no recovery path from the Rust layer.
