...
|---------------|----------|--------|
| [`golang-migrate`](https://github.com/golang-migrate/migrate) | `server/db/migrations/` | users, sessions, accounts, verification_tokens, password_reset_tokens, conversations, messages, documents, document_chunks, ingestion_jobs, chat_messages, user_memories |

Migrations are applied automatically by Docker Compose during startup via a dedicated `migrations` container that runs before all application services. The container uses `golang-migrate` with standard `{timestamp}_{name}.up.sql` / `.down.sql` files.
