...
OpenTier uses a **single PostgreSQL database** shared between both services. Schema management is split:

| Owner | Migration Tool | Tables |
|-------|---------------|--------|
| Rust API | `sqlx-migrate` (in `server/api/migrations/`) | users, sessions, accounts, verification_tokens, password_reset_tokens, conversations, messages |
| Python Intelligence | Manual/Alembic (in `server/intelligence/migrations/`) | documents, document_chunks, ingestion_jobs, chat_messages, user_memories |
| Shared | — | `conversations` (written by Rust, read by both) |

Migrations run in sequence. Python migrations depend on the `conversations` table existing (created by Rust migration `20260114000001`).
