...
**Migration tool**: [`golang-migrate`](https://github.com/golang-migrate/migrate) — a lightweight, standalone CLI that reads standard `{timestamp}_{name}.up.sql` / `.down.sql` files.

**Docker Compose integration**: a dedicated `migrations` service in `server/docker-compose.yml` runs `migrate up` after the database passes its healthcheck and before any application service starts. The startup order is enforced via `depends_on` conditions:

```
db (service_healthy) → migrations (service_completed_successfully) → intelligence → api
```
