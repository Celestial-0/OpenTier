...
**Significance of ordering**: `AdminProvider` reads `user.role` from `AuthProvider`, so it must be nested inside it. `UiProvider` reads nothing from auth — it is purely UI state. `Toaster` is placed inside `AuthProvider` so auth-triggered toasts are rendered correctly.
