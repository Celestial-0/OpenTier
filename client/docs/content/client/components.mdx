...
- Sub-views: `overview.tsx` (stats), `conversations.tsx` (user's conversations), `sessions.tsx` (active sessions), `notifications.tsx`, `profile.tsx`, `settings.tsx`, `admin.tsx` (admin-only: user management + resources), `ingestion-queue.tsx` (real-time task tracking)
